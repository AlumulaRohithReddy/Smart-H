...
export const Attendance = () => {
  ...
  const [attendli,setattendli]=useState()
   ...
   const getAttendance=async()=>{
    const response=await fetch(`https://smart-h-backend-k5oz.onrender.com/api/a/attend`,{
      method:'get',
      headers:{
...
const myFile = new File([jpegBlob], 'myFile.jpg', {
 ...
 const dataTransfer = new DataTransfer();
    ...
    axios.post(`https://smart-h-backend-k5oz.onrender.com/api/a/attend`, formData, {headers: {
      'auth-token': localStorage.getItem('token')
      }})
